...
class SWATRun:
    ...
    """
    def __init__(self, swat_exe_path: str):
        self.swat_exe_path = Path(swat_exe_path)
        if not self.swat_exe_path.exists():
            raise FileNotFoundError(f"SWAT exe not found: {self.swat_exe_path}")

    def run(self, txinout_path: str, capture_output: bool = False):
        txinout_path = Path(txinout_path)

        # Validate
        if not txinout_path.exists():
            raise FileNotFoundError(f"TxtInOut not found: {txinout_path}")
